...
public class JoinWorkersToExistingSwarmPlanConfig extends ParentPlanConfig {
    ...
    @Override
    public Plan getPlan() {
        return new Plan(new Project().name(PROJECT_NAME).key(getProjectKey()).description(DEFAULT_DESCRIPTION), "Join workers to existing Swarm", getPlanKey()).description(DEFAULT_DESCRIPTION)
                        ...
                        .jobs(new Job("Default Job", new BambooKey("JOB1"))
                                .tasks(new VcsCheckoutTask()
                                        ...
                                        new ScriptTask()
                                                ...
                                                .inlineBody(ResourceReader.loadAsString("/scripts/join_workers_to_existing_swarm.sh"))
